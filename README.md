...
- **Web access** — per-agent toggle using OpenRouter's server-side web-search plugin: the model's answer is grounded with live search results and the sources render as citation links under the reply. No CORS issues because the search runs on OpenRouter's side (~0,02 EUR per search, OpenRouter models only).
